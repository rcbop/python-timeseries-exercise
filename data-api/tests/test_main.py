...
def test_main(router_mock: Mock, fast_api_mock: Mock, uvicorn_mock: Mock):
    ...
    app_mock.include_router.assert_called_once_with(
        router_mock, tags=["sensors", "temperature", "humidity"], prefix="/sensors")
    uvicorn_mock.assert_called_once_with(
        app_mock, host="0.0.0.0", port=8000, log_level="info", proxy_headers=True)
